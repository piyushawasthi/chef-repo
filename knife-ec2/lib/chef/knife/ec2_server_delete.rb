    ...
    class Ec2ServerDelete < Knife
      ...
      def run
        ...
        if config[:delete_all]
          # code to remove all instance of ec2
          begin
            total_server = connection.servers
            puts '\n'
            confirm('Do you really want to delete all servers?')
            total_server.each do |server|
              # server.destroy
            end
            ui.warn("Successfully deleted all Servers.")
          rescue Exception => e
